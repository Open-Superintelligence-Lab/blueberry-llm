...
class MLA(nn.Module):
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor]):
        ...
        # Update cache without gradients, but use original tensors for computation
        with torch.no_grad():
            self.k_cache[:bsz, start_pos:end_pos] = k
            self.v_cache[:bsz, start_pos:end_pos] = v
        
        # Build k and v tensors for attention computation
        if start_pos == 0:
            # First position, only use current k, v
            k_attn = k
            v_attn = v
        else:
            # Concatenate historical cache with current k, v
            k_attn = torch.cat([self.k_cache[:bsz, :start_pos], k], dim=1)
            v_attn = torch.cat([self.v_cache[:bsz, :start_pos], v], dim=1)
        
        scores = torch.einsum("bshd,bthd->bsht", q, k_attn) * self.softmax_scale
        
        if mask is not None:
            scores += mask.unsqueeze(1)
        scores = scores.softmax(dim=-1, dtype=torch.float32).type_as(x)
        
        x = torch.einsum("bsht,bthd->bshd", scores, v_attn)
