...
class MixtureOfExperts(nn.Module):
    ...
    def __init__(
        ...
        dropout: float = 0.1,
        load_balancing_weight: float = 0.01,
        activation: str = "silu"  # Add this parameter
    ):
        super().__init__()
        self.num_experts = num_experts
        self.top_k = top_k
        ...
        # Create experts with proper parameter passing
        self.experts = nn.ModuleList([
            Expert(d_model, d_ff, activation, dropout) for _ in range(num_experts)
        ])
...
class MoETransformerBlock(nn.Module):
    def __init__(
        ...
        # MoE layer with activation parameter
        self.feed_forward = MixtureOfExperts(
            d_model, d_ff, num_experts, top_k, dropout, activation=activation
        )
...
class MoEMinimalLLM(nn.Module):
    def __init__(self, config: MoEModelConfig):
        ...
        # Get activation from config, default to "silu"
        ...
        self.transformer_blocks = nn.ModuleList([
            MoETransformerBlock(
                ...
                config.expert_top_k,
                config.dropout,
                activation  # Pass the activation
            )
...
def train_moe_model(config: MoEModelConfig, train_loader: DataLoader, val_loader: DataLoader):
    ...
    while step < config.max_steps:
        for batch_idx, (x, y) in enumerate(train_loader):
            ...
            aux_losses.append(aux_loss.item() if aux_loss is not None else 0.0)
    ...
    print(f"\n📊 Final Results:{config.activation}")
    print(f"   Val Loss: {final_eval['val_loss']:.4f}")
    print(f"   Val Accuracy: {final_eval['val_accuracy']:.4f}")
    print(f"   Val Perplexity: {final_eval['val_perplexity']:.2f}")

    # -----------------------------
    # Return model, final metrics, and training curves
    # -----------------------------
    history = {
    ...
    return model, final_eval, history